...
import { AudioVideoMediaStatus } from './audio-video-media.vo';
...
export class Video extends AggregateRoot {
  ...
  static create(props: VideoCreateCommand) {
    const video = new Video({
      ...
      cast_members_id: new Map(
        props.cast_members_id.map((id) => [id.value, id]),
      ),
      is_published: false,
    });
  ...
  toJSON() {
    return {
      ...
      genres_id: Array.from(this.genres_id.values()).map((id) => id.value),
