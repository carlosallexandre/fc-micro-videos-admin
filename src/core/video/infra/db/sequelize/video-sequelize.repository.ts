...
export class VideoSequelizeRepository implements IVideoRepository {
  ...
  async bulkInsert(entities: Video[]): Promise<void> {
    ...
    entities.map((e) => this.uow.addAggregateRoot(e));
  }
  ...
  async update(entity: Video): Promise<void> {
    const model = await this._get(entity.video_id.value);

    if (!model) {
      throw new NotFoundError(entity.video_id, this.getEntity());
    }
    ...
    await this.videoModel.update(props, {
      where: { video_id: entity.video_id.value },
      transaction: this.uow.getTransaction(),
    });
  ...
  async search(props: VideoSearchParams): Promise<VideoSearchResult> {
    ...
    if (
      ...
      if (props.filter.genres_id) {
        wheres.push({
          field: 'genres_id',
          value: props.filter.genres_id.map((c) => c.value),
          get condition() {
            return {
              ['$genres_id.genre_id$']: {
                [Op.in]: this.value,
      ...
      if (props.filter.cast_members_id) {
        wheres.push({
          ...
          value: props.filter.cast_members_id.map((c) => c.value),
          get condition() {
            return {
